...
Serialize.deserializeAction = Serialize.deserializeActionData = Serialize.serializeAction = Serialize.serializeActionData = Serialize.transactionHeader = Serialize.getTypesFromAbi = Serialize.getType = Serialize.createInitialTypes = Serialize.hexToUint8Array = Serialize.arrayToHex = Serialize.symbolToString = Serialize.stringToSymbol = Serialize.blockTimestampToDate = Serialize.dateToBlockTimestamp = Serialize.timePointSecToDate = Serialize.dateToTimePointSec = Serialize.timePointToDate = Serialize.dateToTimePoint = Serialize.supportedAbiVersion = Serialize.SerialBuffer = Serialize.SerializerState = void 0;
...
Serialize.SerializerState = SerializerState;
...
Serialize.SerialBuffer = SerialBuffer;
/** Is this a supported ABI version? */
Serialize.supportedAbiVersion = function (version) {
...
Serialize.dateToTimePoint = function (date) {
    return Math.round(checkDateParse(date + 'Z') * 1000);
};
/** Convert `time_point` (miliseconds since epoch) to date in ISO format */
Serialize.timePointToDate = function (us) {
    var s = (new Date(us / 1000)).toISOString();
    return s.substr(0, s.length - 1);
};
/** Convert date in ISO format to `time_point_sec` (seconds since epoch) */
Serialize.dateToTimePointSec = function (date) {
    return Math.round(checkDateParse(date + 'Z') / 1000);
};
/** Convert `time_point_sec` (seconds since epoch) to to date in ISO format */
Serialize.timePointSecToDate = function (sec) {
    var s = (new Date(sec * 1000)).toISOString();
    return s.substr(0, s.length - 1);
};
/** Convert date in ISO format to `block_timestamp_type` (half-seconds since a different epoch) */
Serialize.dateToBlockTimestamp = function (date) {
    return Math.round((checkDateParse(date + 'Z') - 946684800000) / 500);
};
/** Convert `block_timestamp_type` (half-seconds since a different epoch) to to date in ISO format */
Serialize.blockTimestampToDate = function (slot) {
    var s = (new Date(slot * 500 + 946684800000)).toISOString();
    return s.substr(0, s.length - 1);
};
/** Convert `string` to `Symbol`. format: `precision,NAME`. */
Serialize.stringToSymbol = function (s) {
    if (typeof s !== 'string') {
...
Serialize.symbolToString = function (_a) {
    var name = _a.name, precision = _a.precision;
    return precision + ',' + name;
};
/** Convert binary data to hex */
Serialize.arrayToHex = function (data) {
    var e_1, _a;
    var result = '';
    try {
...
Serialize.hexToUint8Array = function (hex) {
...
Serialize.createInitialTypes = function () {
    var result = new Map(Object.entries({
        ...
        float128: createType({
            name: 'float128',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(Serialize.hexToUint8Array(data), 16); },
            deserialize: function (buffer) { return Serialize.arrayToHex(buffer.getUint8Array(16)); },
        }),
        bytes: createType({
            name: 'bytes',
            serialize: function (buffer, data) {
                if (data instanceof Uint8Array || Array.isArray(data)) {
                    ...
                    buffer.pushBytes(Serialize.hexToUint8Array(data));
                }
            },
            deserialize: function (buffer, state) {
                if (state && state.options.bytesAsUint8Array) {
                    ...
                    return Serialize.arrayToHex(buffer.getBytes());
                }
            },
        }),
        string: createType({
        ...
        time_point: createType({
            name: 'time_point',
            serialize: function (buffer, data) { buffer.pushNumberAsUint64(Serialize.dateToTimePoint(data)); },
            deserialize: function (buffer) { return Serialize.timePointToDate(buffer.getUint64AsNumber()); },
        }),
        time_point_sec: createType({
            name: 'time_point_sec',
            serialize: function (buffer, data) { buffer.pushUint32(Serialize.dateToTimePointSec(data)); },
            deserialize: function (buffer) { return Serialize.timePointSecToDate(buffer.getUint32()); },
        }),
        block_timestamp_type: createType({
            name: 'block_timestamp_type',
            serialize: function (buffer, data) { buffer.pushUint32(Serialize.dateToBlockTimestamp(data)); },
            deserialize: function (buffer) { return Serialize.blockTimestampToDate(buffer.getUint32()); },
        }),
        ...
        symbol: createType({
            name: 'symbol',
            serialize: function (buffer, data) { buffer.pushSymbol(Serialize.stringToSymbol(data)); },
            deserialize: function (buffer) { return Serialize.symbolToString(buffer.getSymbol()); },
        }),
        asset: createType({
            name: 'asset',
        ...
        checksum160: createType({
            name: 'checksum160',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(Serialize.hexToUint8Array(data), 20); },
            deserialize: function (buffer) { return Serialize.arrayToHex(buffer.getUint8Array(20)); },
        }),
        checksum256: createType({
            name: 'checksum256',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(Serialize.hexToUint8Array(data), 32); },
            deserialize: function (buffer) { return Serialize.arrayToHex(buffer.getUint8Array(32)); },
        }),
        checksum512: createType({
            name: 'checksum512',
            serialize: function (buffer, data) { buffer.pushUint8ArrayChecked(Serialize.hexToUint8Array(data), 64); },
            deserialize: function (buffer) { return Serialize.arrayToHex(buffer.getUint8Array(64)); },
        }),
...
/** Get type from `types` */
Serialize.getType = function (types, name) {
    var type = types.get(name);
    if (type && type.aliasOfName) {
        return Serialize.getType(types, type.aliasOfName);
    }
    if (type) {
        return type;
    }
    if (name.endsWith('[]')) {
        return createType({
            name: name,
            arrayOf: Serialize.getType(types, name.substr(0, name.length - 2)),
    ...
    if (name.endsWith('?')) {
        return createType({
            name: name,
            optionalOf: Serialize.getType(types, name.substr(0, name.length - 1)),
    ...
    if (name.endsWith('$')) {
        return createType({
            name: name,
            extensionOf: Serialize.getType(types, name.substr(0, name.length - 1)),
 ...
 */
Serialize.getTypesFromAbi = function (initialTypes, abi) {
    ...
    try {
        for (var types_1 = __values(types), types_1_1 = types_1.next(); !types_1_1.done; types_1_1 = types_1.next()) {
            ...
            if (type.baseName) {
                type.base = Serialize.getType(types, type.baseName);
            }
            try {
                for (var _r = (e_9 = void 0, __values(type.fields)), _s = _r.next(); !_s.done; _s = _r.next()) {
                    var field = _s.value;
                    field.type = Serialize.getType(types, field.typeName);
                }
            }
...
Serialize.transactionHeader = function (refBlock, expireSeconds) {
    ...
    return {
        expiration: Serialize.timePointSecToDate(Serialize.dateToTimePointSec(timestamp) + expireSeconds),
        ref_block_num: refBlock.block_num & 0xffff,
        ref_block_prefix: prefix,
    };
};
/** Convert action data to serialized form (hex) */
Serialize.serializeActionData = function (contract, account, name, data, textEncoder, textDecoder) {
...
Serialize.serializeAction = function (contract, account, name, authorization, data, textEncoder, textDecoder) {
    return {
        account: account,
        name: name,
        authorization: authorization,
        data: Serialize.serializeActionData(contract, account, name, data, textEncoder, textDecoder),
    };
};
/** Deserialize action data. If `data` is a `string`, then it's assumed to be in hex. */
Serialize.deserializeActionData = function (contract, account, name, data, textEncoder, textDecoder) {
    var action = contract.actions.get(name);
...
Serialize.deserializeAction = function (contract, account, name, authorization, data, textEncoder, textDecoder) {
    return {
        account: account,
        name: name,
        authorization: authorization,
